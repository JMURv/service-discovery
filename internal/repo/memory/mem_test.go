...
func TestRepository(t *testing.T) {
	...
	t.Run(
		"Deregister services", func(t *testing.T) {
	...
	t.Run(
		"List addresses for a service", func(t *testing.T) {
			addrs, err := r.ListAddrsByName(ctx, "service2")
			assert.NoError(t, err)
			assert.ElementsMatch(t, []string{"addr3", "addr4"}, addrs)

			addrs, err = r.ListAddrsByName(ctx, "non-existing-service")
			assert.Equal(t, repo.ErrNotFound, err)
